...
class AFPINetTrainerWithMixup(BaseTrainer):
    """
    Trainer class
    """
    def __init__(self, torch_args: dict, save_dir, resume, device, **kwargs):
        ...
        ### Load model weights and set grad to false for feature extractors
        spec_weights = torch.load("saved/dcase_task1b_1sec_spec/Spec Static LR/model/model_best.pth")['models']['model']
        wave_weights = torch.load("saved/dcase_task1b_1sec_wave/Wave Static LR/model/model_best.pth")['models']['model']
        
        from collections import OrderedDict
        new_spec = OrderedDict()

        for key, value in spec_weights.items():
            if "tail" not in key:
                new_key = "spec." + key
                new_spec[new_key] = value

        new_wave = OrderedDict()
        for key, value in wave_weights.items():
            if "tail" not in key:
                new_key = "wave." + key
                new_wave[new_key] = value

        self.models['model'].load_state_dict(new_spec, strict=False)
        self.models['model'].load_state_dict(new_wave, strict=False)
        
        for name, param in self.models['model'].named_parameters():
            if "wave" in name or "spec" in name:
                param.requires_grad = False
        self.logger.debug("Set weights of feature extractors and set requires_grad to false.")
