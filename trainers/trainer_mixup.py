...
class AFPINetTrainerWithMixup(BaseTrainer):
    ...
    def __init__(self, torch_args: dict, save_dir, resume, device, **kwargs):
        ...
        if self.len_epoch is None:
            ...
            self.len_epoch = len(self.train_data_loaders['data'])  # data_size/batch_size  ==> batch num in each epoch
        else:
        ...
        # ## Calculate the learning rate before formal training
        if kwargs.get('find_lr'):
            ...
            if kwargs.get('max_lr'):
                ...
                max_lr = np.median([self._find_lr() for _ in range(7)])
