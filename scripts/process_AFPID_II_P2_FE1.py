...
ori_data_dir = "../data/acoustic_footstep/AFPID-II_P2-Raw"


cloth_train_df = pd.DataFrame(columns=["filename_wave", "filename_hcraft", "filename_spec", "person_label"])
cloth_test_df = pd.DataFrame(columns=["filename_wave", "filename_hcraft", "filename_spec", "person_label"])
shoe_train_df = pd.DataFrame(columns=["filename_wave", "filename_hcraft", "filename_spec", "person_label"])
shoe_test_df = pd.DataFrame(columns=["filename_wave", "filename_hcraft", "filename_spec", "person_label"])

# generated dataset path
afpid_dataset_dir = "../data/acoustic_footstep/AFPID-II_P2_FE1"
...
f = h5py.File(os.path.join(ori_data_dir, 'afpid_ii_p2_fse_sep_tim_vec.mat'), 'r')
...
for i in range(41):
    ...
    for j in range(len(audio_dir)):
        ...
        fse_sep_tim_vec = np.squeeze(fse_sep_tim_vec_cell[(i-1) * 4 + j]).astype(int)
        # load audio
        ...
        for k in range(4, len(fse_sep_tim_vec) - 2):
            ...
            fname_wave = os.path.join(wave_cachedir, f"afpid_ii_p2_fe1_s{sample_fname}_waveform_{sample_num}.npy")
            fname_hcraft = os.path.join(hcraft_cachedir, f"afpid_ii_p2_fe1_s{sample_fname}_handcraft_{sample_num}.npy")
            fname_spec = os.path.join(spec_cachedir, f"afpid_ii_p2_fe1_s{sample_fname}_spectrogram_{sample_num}.npy")
            sample_num += 1

            # saving
            # np.save(os.path.join(self.spectrogram_cachedir, item['filename_audio']).replace(".wav", ".npy"), spec)
            np.save(os.path.join(afpid_dataset_dir, fname_wave), padded)
            ...
            if session == 1:
                cloth_train_df = cloth_train_df.append(
                ...
                shoe_train_df = shoe_train_df.append(
                    {"filename_wave": fname_wave, "filename_hcraft": fname_hcraft, "filename_spec": fname_spec,
                     "person_label": f"S{sample_fname[:2]}"}, ignore_index=True)
            elif session == 2:
                cloth_test_df = cloth_test_df.append(
                    {"filename_wave": fname_wave, "filename_hcraft": fname_hcraft, "filename_spec": fname_spec,
                     "person_label": f"S{sample_fname[:2]}"}, ignore_index=True)
                shoe_train_df = shoe_train_df.append(
                    {"filename_wave": fname_wave, "filename_hcraft": fname_hcraft, "filename_spec": fname_spec,
                ...
                cloth_test_df = cloth_test_df.append(
                    {"filename_wave": fname_wave, "filename_hcraft": fname_hcraft, "filename_spec": fname_spec,
                     "person_label": f"S{sample_fname[:2]}"}, ignore_index=True)
                shoe_test_df = shoe_test_df.append(
                    {"filename_wave": fname_wave, "filename_hcraft": fname_hcraft, "filename_spec": fname_spec,
                     "person_label": f"S{sample_fname[:2]}"}, ignore_index=True)
            elif session == 4:
                cloth_train_df = cloth_train_df.append(
...
cloth_train_df.to_csv(os.path.join(afpid_dataset_dir, 'AFPID_II_P2_FE1_cloth_train.csv'))
cloth_test_df.to_csv(os.path.join(afpid_dataset_dir, 'AFPID_II_P2_FE1_cloth_test.csv'))
shoe_train_df.to_csv(os.path.join(afpid_dataset_dir, 'AFPID_II_P2_FE1_shoe_train.csv'))
shoe_test_df.to_csv(os.path.join(afpid_dataset_dir, 'AFPID_II_P2_FE1_shoe_test.csv'))

print("Finished to create the AFPID_FE1 dataset with two consecutive footstep events to form on sample!")
